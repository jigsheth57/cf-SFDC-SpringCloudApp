...
sfdcApp.controller('AccountListController', function($scope, $http, $interval) {
    ...
    $scope.getAccountServiceStatus = function() {
        ...
        $http.get('/health').success(function(data) {
            console.log("response data: " + JSON.stringify(data));
            var serviceStatus = JSON.stringify(data.components.discoveryComposite.components.discoveryClient.details.services);
            var sfdcserviceStatus = JSON.stringify(data.components.SFDC.status);
